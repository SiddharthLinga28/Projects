...
class BaseVFL(object):
    ...
    def split_custom(self, image):
        ...
        if self.args.num_passive == 2:
            # Party 0: center
            mask_center = torch.zeros_like(image)
            mask_center[:, :, h_third:2*h_third, w_third:2*w_third] = 1
            data.append(image * mask_center)

            # Party 1: all other areas
            mask_other = torch.ones_like(image)
            mask_other[:, :, h_third:2*h_third, w_third:2*w_third] = 0
            data.append(image * mask_other)

        elif self.args.num_passive == 3:
            # Party 0: center
            mask_center = torch.zeros_like(image)
            mask_center[:, :, h_third:2*h_third, w_third:2*w_third] = 1
            data.append(image * mask_center)
            ...
            # Party 1: corners
            mask_corners = torch.zeros_like(image)
            coords = [
                (0, h_third, 0, w_third),
                (0, h_third, 2*w_third, W),
                (2*h_third, H, 0, w_third),
                (2*h_third, H, 2*w_third, W)
            ]
            for top, bottom, left, right in coords:
                mask_corners[:, :, top:bottom, left:right] = 1
            data.append(image * mask_corners)

            # Party 2: edges
            mask_edges = torch.ones_like(image)
            mask_edges -= mask_center
            mask_edges -= mask_corners
            data.append(image * mask_edges)

        else:
            raise ValueError("Custom division supports only 2 or 3 passive parties.")
    ...
    def _process_data(self, train_dataset, test_dataset):
        if self.args.padding_mode:
            ...
            for batch_data in train_dataset:
                ...
                for i in range(self.args.num_passive):
                    if i == self.args.attack_id:
                        ...
                        train_data_padded.append(torch.rand(data.shape))
        ...
        elif self.args.division_mode == 'custom':
            self.train_dataset = []
            self.train_dataset_len = 0
            for batch_data in train_dataset:
                data, labels = batch_data
                custom_split = self.split_custom(data)
                self.train_dataset.append([custom_split, labels])
                self.train_dataset_len += len(data)
            ...
            for batch_data in test_dataset:
                data, labels = batch_data
                custom_split = self.split_custom(data)
                self.test_dataset.append([custom_split, labels])
                self.test_dataset_len += len(data)
