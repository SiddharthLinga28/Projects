...
class Conv4_passive(nn.Module):
    ...
    def forward(self, x):
      if hasattr(self, 'args') and self.args.division_mode == 'custom':
          N, C, H, W = x.shape
          h_third, w_third = H // 3, W // 3

          if self.args.num_passive == 2:
              if self.party_index == 0:  # center
                  x = x[:, :, h_third:2*h_third, w_third:2*w_third]
              elif self.party_index == 1:  # outer
                  mask = torch.ones_like(x)
                  mask[:, :, h_third:2*h_third, w_third:2*w_third] = 0
                  x = x * mask
          elif self.args.num_passive == 4:
              if self.party_index == 0:  # center
                  x = x[:, :, h_third:2*h_third, w_third:2*w_third]
              elif self.party_index == 1:  # corners
                  mask = torch.zeros_like(x)
                  mask[:, :, :h_third, :w_third] = 1
                  mask[:, :, :h_third, 2*w_third:] = 1
                  mask[:, :, 2*h_third:, :w_third] = 1
                  mask[:, :, 2*h_third:, 2*w_third:] = 1
                  x = x * mask
              elif self.party_index == 2:  # edges (excluding corners and center)
                  mask = torch.ones_like(x)
                  mask[:, :, h_third:2*h_third, w_third:2*w_third] = 0  # remove center
                  mask[:, :, :h_third, :w_third] = 0
                  mask[:, :, :h_third, 2*w_third:] = 0
                  mask[:, :, 2*h_third:, :w_third] = 0
                  mask[:, :, 2*h_third:, 2*w_third:] = 0
                  x = x * mask
              else:  # any extra party
...
class Conv4(nn.Module):
    ...
    '''
    def __init__(self, num_classes, num_passive, padding_mode, division_mode):
        ...
        for i in range(num_passive):
            ...
            self.passive[i].args = args
            self.passive[i].party_index = i
...
class FC1_active(nn.Module):
    '''
    Conv4 active model for MNIST and FashionMNIST.
    28 * 28 * 1
    '''
    def __init__(self):
...
class FC1(nn.Module):
    ...
    28 * 28 * 1
    '''
    def __init__(self, num_passive, padding_mode, division_mode):
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, num_passive, padding_mode, division_mode):
...
class DeepFM(nn.Module):
    def __init__(self, feature_sizes, emb_size, hidden_size, dropout, num_classes, num_passive, padding_mode, division_mode):
...
entire_simple = {
    ...
    'cifar100': partial(Conv4, num_classes=100),
    'criteo': partial(DeepFM, feature_sizes=feature_sizes, emb_size=4, hidden_size=32, dropout=0.5, num_classes=2)
}